...
public class CreditCheckServiceBean implements CreditCheckService {
    ...
    @Inject
    @Reference("RuleService")
    private RuleService checkRule;

    @Inject
    @Reference("ApplicationLoggerReference")
    private ApplicationLogger applicationLoggerJms;

    @Inject
    @Reference("ApplicationLoggerReference2")
    private ApplicationLogger applicationLoggerJpa;

    @Override
    public Application checkCredit(Offer offer) {
        Application app = checkRule.checkCredit(offer);
        applicationLoggerJms.store(app);
        applicationLoggerJpa.store(app);
        return app;
    }
